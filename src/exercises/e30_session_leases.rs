//! Exercise 30 — see who holds what: `aida session leases` lists the
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida session leases — who holds what"
    }
    ...
    fn verify(&self, workspace: &Path) -> VerifyResult {
        ...
        if session_lease_for_branch(workspace, SESSION_BRANCH).is_some() {
            ...
            VerifyResult::Pending(
                "no active session lease to inspect — complete exercise 28 first, and don't \
                 end the session until after this exercise."
                    .into(),
            )
        }
    }
