//! Exercise 21 — the cache is disposable; rebuild it from the git store.
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida cache rebuild — the cache is disposable"
    }
        ...
        if !cache_db_is_valid_sqlite(workspace) {
            return VerifyResult::Fail(
                "`.aida/cache.db` is missing or not a valid SQLite database. Run \
                 `aida cache rebuild` to regenerate it from the git store."
                    .into(),
            );
        }
        VerifyResult::Pass
