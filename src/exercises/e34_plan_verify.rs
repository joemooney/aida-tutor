//! Exercise 34 — `aida plan verify`: lint an implementation plan archived
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida plan verify — lint an implementation plan"
    }
    ...
    fn verify(&self, workspace: &Path) -> VerifyResult {
        ...
        if plans.is_empty() {
            return VerifyResult::Pending(
                "no plan under `docs/plans/` yet — write one and run \
                 `aida plan verify docs/plans/<file>.md`."
                    .into(),
            );
        }
        ...
        VerifyResult::Fail(
            "a plan exists under `docs/plans/` but it's missing a required section — \
             ...
             `## Followups` is absent. Add the missing heading(s) and re-verify."
                .into(),
        )
    }
...
## Related
- FR-1 — the feature captured in exercise 02
";
