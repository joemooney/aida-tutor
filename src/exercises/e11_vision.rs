//! Exercise 11 — capture project vision (VIS). trace:STORY-2 | ai:claude
...
impl Exercise for E {
    ...
    fn verify(&self, workspace: &Path) -> VerifyResult {
        if !is_aida_initialized(workspace) {
            return VerifyResult::Pending("complete exercise 01 first (no AIDA store yet)".into());
        }
        ...
        if !approved {
            return VerifyResult::Fail(
                "VIS-* found but its status is not `approved`. Visions are declarations — \
                 use `aida edit VIS-1 --status approved` to advance it."
                    .into(),
            );
        }
        VerifyResult::Pass
    ...
    fn demo(&self, workspace: &Path) -> anyhow::Result<()> {
        run(
            ...
            &[
                ...
                "approved",
                "--title",
                "Ship a delightful command-line task tracker",
            ],
        )
    }
}
