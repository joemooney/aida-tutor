//! Exercise 25 — finish a queued item: `aida queue done` completes the
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida queue done — finish + dequeue atomically"
    }
    ...
    fn hint_more(&self) -> Option<&'static str> {
        Some(
            "1. The \"Queue demo\" task is in-progress from exercise 24.\n\
        ...
        if still_queued {
            return VerifyResult::Fail(
                "the \"Queue demo\" task is done but still sitting in the queue — \
                 `aida queue done` should have removed it. Try `aida queue remove <ID>`."
                    .into(),
            );
        }
        VerifyResult::Pass
