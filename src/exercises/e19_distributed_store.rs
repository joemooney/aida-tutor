//! Exercise 19 — the distributed store: orphan branch + linked worktree.
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "the distributed store — orphan branch + worktree"
    }
        ...
        if !git_branch_exists(workspace, "aida-store") {
            return VerifyResult::Fail(
                "no `aida-store` branch in workspace/ — the orphan store branch is missing. \
                 `aida init` should have created it."
                    .into(),
            );
        }
        if !is_linked_worktree(workspace, ".aida-store") {
            return VerifyResult::Fail(
                "`.aida-store/` is not a linked git worktree (no `.git` pointer file). The \
                 distributed store is a worktree checked out on the `aida-store` branch."
                    .into(),
            );
        }
