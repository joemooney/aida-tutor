//! Exercise 24 — pick up a queued item: peek the head, claim it by
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida queue next — pick up the next item"
    }
    ...
    fn verify(&self, workspace: &Path) -> VerifyResult {
        ...
        let Some(task) = requirement_by_title(workspace, QUEUE_DEMO_NEEDLE) else {
            return VerifyResult::Pending(
                "no \"Queue demo\" task in the store — complete exercise 23 first.".into(),
            );
        };
    ...
    fn demo(&self, workspace: &Path) -> anyhow::Result<()> {
        // Consumer side: wear the implementer hat (exercise 23's producer
        // demo switched this process to advisor), peek the queue head,
        // then claim the cluster's task by flipping it in-progress.
        // `queue next` filters by the active role. The task is already
        // `approved` (the advisor created it so), so Approved → InProgress
        // needs no special authority. trace:STORY-46
