...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida session end — release the worktree"
    }
    ...
    fn verify(&self, workspace: &Path) -> VerifyResult {
        ...
        // branch means exercise 28 hasn't been done.
        if !git_branch_exists(workspace, SESSION_BRANCH) {
            return VerifyResult::Pending(
                "no `session-work` branch — complete exercise 28 first.".into(),
            );
        }
