//! Exercise 05 — write code with a trace comment. trace:STORY-10 | ai:claude
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "write code with a trace:FR-1 comment"
    }
        ...
        if fr_traces.is_empty() {
            return VerifyResult::Pending(
                "no `trace:FR-...` comment found in any file under workspace/ — \
                 add one in any source file (Rust, Python, JS, anything)"
                    .into(),
            );
        }
        VerifyResult::Pass
