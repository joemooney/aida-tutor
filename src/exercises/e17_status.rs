//! Exercise 17 — `aida status`. trace:STORY-16 | ai:claude
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida status — read the project pulse"
    }
    ...
    fn verify(&self, workspace: &Path) -> VerifyResult {
        ...
        // Read-only command — see exercise 03: the invocation-logging
        ...
        verify_invocation(
            ...
            "status",
            &[],
            "the invocation wrapper is installed but shows no `aida status` yet — run it",
        )
    }
