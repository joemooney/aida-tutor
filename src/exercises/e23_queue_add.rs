//! Exercise 23 — route work to a role's queue: the producer side of the
...
/// The exact title the queue-cluster task carries. Exercises 23-25 follow
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida queue add --for — route work to a role"
    }
        ...
        let Some(task) = requirement_by_title(workspace, QUEUE_DEMO_NEEDLE) else {
            return VerifyResult::Pending(
                "no \"Queue demo\" task captured yet — run `aida add --type task --status \
                 approved --title \"Queue demo: ship the sample widget\"`."
                    .into(),
            );
        };
    ...
    fn demo(&self, workspace: &Path) -> anyhow::Result<()> {
        // Producer side: routing work (and capturing it as `approved`) is
        // an advisor action under AIDA's authority model. Exercise 22's
        // demo left this process wearing the `implementer` hat; switch to
        // advisor for the producer steps. A real learner working
        // interactively already has this authority. trace:STORY-46
        std::env::set_var("AIDA_SESSION_ROLE", "advisor");
        // Capture the cluster's task, then route it to the implementer
        // queue. The spec_id depends on creation order, so we resolve it
        // back from the title rather than hardcoding it.
        run(
            workspace,
            "aida",
            &[
