//! Exercise 29 — work in the session worktree: commit on the session
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "work in the worktree — an isolated branch"
    }
    ...
    fn verify(&self, workspace: &Path) -> VerifyResult {
        ...
        if !git_branch_exists(workspace, SESSION_BRANCH) {
            return VerifyResult::Pending(
                "no session branch yet — complete exercise 28 (`aida session start`) first.".into(),
            );
        }
        ...
        match git_commits_ahead(workspace, &base, SESSION_BRANCH) {
            ...
            None => {
                VerifyResult::Fail("git couldn't count the commits on the session branch.".into())
            }
        }
    }
