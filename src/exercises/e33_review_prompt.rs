//! Exercise 33 — `aida review prompt`: collect requirements' acceptance
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida review prompt — specs into a review brief"
    }
    fn hint(&self) -> &'static str {
        "`aida review prompt --specs <ids>` gathers the acceptance criteria of one or more \
         ...
         exercise 02 and capture the output to a file with `--write`: `aida review prompt \
         --specs FR-1 --write review-brief.md`. Then open `review-brief.md` to read the brief."
    }
    // trace:STORY-20 | ai:claude
    fn hint_more(&self) -> Option<&'static str> {
        Some(
            "1. The feature from exercise 02 is FR-1.\n\
    ...
    fn demo(&self, workspace: &Path) -> anyhow::Result<()> {
        // Brief the feature captured in exercise 02; its spec_id depends
