//! Exercise 28 — start a scoped session: `aida session start` forks a
...
/// The branch name the session cluster pins. Exercises 28-31 follow one
...
impl Exercise for E {
    ...
    fn title(&self) -> &'static str {
        "aida session start — a scoped worktree"
    }
    fn hint(&self) -> &'static str {
        "A *session* claims a slice of work and gives it its own room. `aida session start \
         ...
         recording who owns what. Scope it to the feature from exercise 02 (`FR-1` — confirm \
    ...
    fn hint_more(&self) -> Option<&'static str> {
        Some(
            "1. The feature from exercise 02 is FR-1 — confirm with `aida list --type functional`.\n\
    ...
    fn demo(&self, workspace: &Path) -> anyhow::Result<()> {
        // Scope the session to the feature captured in exercise 02. Its
        // spec_id depends on creation order, so resolve it rather than
        // hardcoding `FR-1`. The branch is pinned so exercises 29-31 can
