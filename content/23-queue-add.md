...
- The **consumer** wears `implementer` (or `reviewer`, …) and pulls
  from *their* queue with `aida queue next` (exercise 24).
...
exact title — exercises 24 and 25 track the same task by it:
...
Exercise 24 — switch to the consumer side and pick the task back up.
